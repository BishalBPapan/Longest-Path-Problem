...
class Colony:
	def __init__(self, edges):
		...
		self.E = edges
		self.N = max(self.V)
		self.init_vert=min(self.V)
		self.dest=max(self.V)
	def build_path(self, probs):
		...
		possible = probs.loc[self.init_vert]
		possible = possible[~possible.index.isin(path + [self.dest])]
		...
		while self.dest not in probs.loc[path[-1]].index:
			path.pop()
